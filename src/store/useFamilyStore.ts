...
interface FamilyState {
  ...
  // Expense actions
  addExpense: (input: { amount: number; categoryName: string; payerMemberId?: string; kidId?: string; date: string; note?: string; isRecurring?: boolean; recurrenceType?: Expense["recurrenceType"]; recurrenceDay?: number }) => Expense;
  updateExpense: (id: string, patch: Partial<Pick<Expense, "amount" | "categoryName" | "payerMemberId" | "kidId" | "date" | "note" | "isRecurring" | "recurrenceType" | "recurrenceDay">>) => void;
...
export const useFamilyStore = create<FamilyState>()(
  persist(
    (set, get) => ({
      ...
      addExpense: ({ amount, categoryName, payerMemberId, kidId, date, note, isRecurring = false, recurrenceType, recurrenceDay }) => {
        const now = Date.now();
        const item: Expense = { id: makeId(), amount, categoryName, payerMemberId, kidId, date, note, isRecurring, recurrenceType, recurrenceDay, updatedAt: now, createdAt: now };
